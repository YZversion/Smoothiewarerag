...
│    → 03_build_repomap.py   → repomap_graph/scores.json  │
└─────────────────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────┐
...
│    └─ search_graph() 或可选 search_reporank()            │
...
### repomap_graph.json / repomap_scores.json

Phase 9 产物，由 `03_build_repomap.py` 构建。图边来源为低噪声 mention、same-file、include、dispatch。运行时只有设置 `ENABLE_REPORANK=1` 或 `--enable-reporank` 时才启用 personalized PageRank；A/B 未达到默认开启门槛，因此默认检索仍走 Phase 8 路径。
...
### Repomap PageRank（Phase 9，可选）

`search_reporank()` 只在 `flow_intent_query()` 且 `ENABLE_REPORANK=1` / `--enable-reporank` 时触发。它以 primary hits、method/class/symbol/dispatch hits、query token 对应 symbol chunks 为 personalized seeds，最多追加 3 个新文件 extras。PageRank 不替代源码证据，不挤掉已有 primary。当前 A/B 结果：全体 mean cov@5 仍为 94%，Q2-Q5 mean cov@5 仍为 68%，未达到 +5pp，因此默认关闭。
