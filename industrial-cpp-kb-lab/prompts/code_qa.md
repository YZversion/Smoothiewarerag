...
## 必须覆盖的 primary 文件（系统自动列出，共 {{primary_count}} 项）
...
- **必须覆盖上方 primary 清单的全部 {{primary_count}} 项**（每项一行，带引用）；若某文件与问题无关，说明为何在 primary 清单中但仍需标注其角色

### 代码片段（可选）
仅当需要展示具体语句时，**逐字复制** context snippet 中的代码（含原有注释），并在代码块**上方或下方**用一行标注来源，格式：来源：`src/path/File.cpp:42-80`。

### 自检（回答结束前核对）
- primary 清单共 **{{primary_count}}** 项；我的「关键文件 / 函数」是否**恰好覆盖**每一项？若有遗漏，在输出前补全。
