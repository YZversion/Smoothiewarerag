...
- [ ] 两阶段 LLM（不需要；检索 + checklist 后 tune 5/5 expected）
...
当前已完成 **Phase 0 – Phase 9**（MVP + Plan B + **CI 绿** + AST-aware 符号 / dispatch + Repomap A/B 默认关闭）；**下一步优先 Phase 10（LLM 答案完整性）或 wire bonder 最小迁移探针**。
...
| `PLAN.md` Phase 10–12 + Plan D | **接下来要做的事与延期实验** |

**路线图顺序：** Phase 7 CI ✅ → Phase 8 AST-aware 符号 / dispatch ✅ → Phase 9 PageRank A/B ✅（默认关闭）→ **10 LLM 完整性** → 11 wire bonder → 12 CLI 产品化。
